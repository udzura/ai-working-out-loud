...
Slack の mrkdwn には**リスト記法がありません**（`- ` はただの文字として表示されます）。そのため
本文は Block Kit の `rich_text` ブロックに変換して投稿し、本物のリストとして表示させています。
対応する記法は WOL の投稿で実際に使う分だけに絞っています。

| 記法 | 結果 |
|---|---|
| 行頭 `- ` / `* ` | 箇条書き |
| 行頭 `1. ` / `1) ` | 番号付きリスト |
| 行頭の空白 2 つ（またはタブ 1 つ）ごとに 1 段 | リストのネスト |
| `` `code` `` | インラインコード |
| `[表示文字](URL)` | リンク |
| 素の `http(s)://…` | リンク（末尾の `。` `)` などは URL から切り離し） |
| その他の行 | 段落（連続行は 1 段落にまとまる） |

太字・斜体は本文では扱いません（見出しと `by <名前>` にのみ使われます）。
`markdown` ブロックは Incoming Webhook では受け付けられない（HTTP 500）ため使っていません。
